...
fn direct_chat_send_read_search_and_tail_work_offline() {
    ...
    assert_eq!(read["data"]["messages"].as_array().unwrap().len(), 1);
    ...
    assert_eq!(tail["data"]["messages"][0]["body"], "queued offline");

    let list = run_iris(alice.path(), &["chat", "list"]);
    assert_eq!(list["data"]["chats"].as_array().unwrap().len(), 1);
    assert_eq!(list["data"]["chats"][0]["last_message"], "queued offline");
