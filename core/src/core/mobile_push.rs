...
pub(crate) use subscriptions::{
    ...
    mobile_push_stored_subscription_id_key, resolve_mobile_push_server_url,
};
...
impl AppCore {
    pub(super) fn build_mobile_push_sync_snapshot(&self) -> MobilePushSyncSnapshot {
        ...
        let background_message_author_pubkeys = self
            ...
            .as_ref()
            .map(|engine| {
                let mut authors = engine.message_author_pubkeys_for_owner(logged_in.owner_pubkey);
                authors.extend(engine.group_sender_event_pubkeys_for_owner(logged_in.owner_pubkey));
                sorted_hexes(authors.into_iter().map(|key| key.to_hex()).collect())
            })
            .unwrap_or_default();
        message_author_pubkeys.extend(background_message_author_pubkeys.iter().cloned());
...
pub(crate) fn decrypt_mobile_push_notification(
    ...
    let event = mobile_push_event_from_payload(&raw_payload_json);
    let mut resolution = decrypt_mobile_push_notification_inner(
        data_dir,
        owner_pubkey_hex,
        device_nsec,
        raw_payload_json,
    ...
    if let (Some(event), Ok(mut payload)) = (
        ...
        if let Some(object) = payload.as_object_mut() {
            ...
            resolution.payload_json = payload.to_string();
        }
    }
    resolution
}

fn decrypt_mobile_push_notification_inner(
    data_dir: String,
    owner_pubkey_hex: String,
    device_nsec: String,
    raw_payload_json: String,
...
fn decrypted_mobile_push_resolution(
    ...
    if sender_owner == local_owner
        || read_state::rumor_is_read(data_dir, &resolved_chat_id, &inner_json)
    {
        return read_state::dismissed_resolution();
    }
