...
#[test]
fn appcore_sender_key_repair_response_duplicate_request_is_throttled() {
    let mut devices = sender_key_matrix_devices(3);
    let alice = 0;
    let bob = 1;
    let carol = 2;
    ...
    let created = devices[alice]
        .engine
        .create_group(
            "sender-key duplicate repair response".to_string(),
            vec![bob_owner_pubkey, carol_owner_pubkey],
            UnixSeconds(360),
        )
        .expect("create sender-key group");
    ...
    let sent = devices[alice]
        ...
        .send_group_payload(
            &group_id,
            b"repair response duplicate throttle".to_vec(),
            Some("sender-key-duplicate-repair-response-inner".to_string()),
        )
        .expect("send with rotated sender key");
    let outer = sender_key_outer_events_for_engine(&devices[alice].engine, &sent.effects, &sent.event_ids)
        .into_iter()
    ...
    let (_alice_events, first_response_effects) =
        deliver_protocol_effects_to_engine_once(&mut devices[alice].engine, &pending.effects);
    assert!(
        !first_response_effects.is_empty(),
        "sender should answer the first repair request"
    );
    let (_alice_events, duplicate_response_effects) =
        deliver_protocol_effects_to_engine_once(&mut devices[alice].engine, &pending.effects);
    assert!(
        duplicate_response_effects.is_empty(),
        "duplicate repair request replay must not create another response publish immediately"
    );
}
