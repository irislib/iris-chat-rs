...
from mixed_app_parity_flow import DEFAULT_PUBLIC_RELAYS
from mixed_group_membership_matrix import run_flow, write_json
from mobile_scenario import Scenario


DEFAULT_SIMULATORS = ["Iris Chat iPhone", "Iris Chat iPhone 2", "Iris Chat iPhone 3"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Same-platform group membership E2E.")
    ...
    parser.add_argument("--serials", help="Three adb serials, space/comma separated. Can be combined with --avds.")
    parser.add_argument("--avds", help="Three Android AVD names, space/comma separated. Can be combined with --serials.")
    parser.add_argument("--simulator", action="append", default=[], help="Simulator name. Pass three times.")
    parser.add_argument("--simulators", help="Three simulator names separated by comma or |.")
    parser.add_argument("--udid", action="append", default=[], help="Simulator/device UDID. Pass three times.")
    parser.add_argument("--udids", help="Three UDIDs separated by comma or |.")
...
def relay_config(args: argparse.Namespace, artifact_dir: Path, *, has_android_serial: bool) -> tuple[dict[str, Any], bool]:
        ...
        return (
            {
                ...
                "set_id": f"{args.platform}-group-membership-public-{artifact_dir.name}",
            },
            False,
        )
    ...
    relay = {
        "start": True,
        "port": port,
        "label": f"iris.{args.platform}-group-membership.{artifact_dir.name}.relay",
        "drop_file": str(artifact_dir / "scenario" / "drop-events.txt"),
        "log_file": str(artifact_dir / "scenario" / "relay.log"),
        "set_id": f"{args.platform}-group-membership-{artifact_dir.name}",
        "bind_host": "0.0.0.0",
        "url": ios_url if args.platform == "ios" else android_url,
        "android_url": android_url,
    }
    return relay, relay_uses_localhost(android_url)
...
def ios_devices(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    if len(udids) < 3 and len(simulators) < 3:
        simulators = DEFAULT_SIMULATORS
    use_udids = len(udids) >= 3
...
def android_targets(args: argparse.Namespace) -> list[dict[str, str]]:
    ...
    selected: list[dict[str, str]] = [{"serial": serial} for serial in serials[:3]]
    selected.extend({"avd": avd} for avd in avds[: 3 - len(selected)])
    if len(selected) < 3:
        raise SystemExit(
            "Need three Android targets. Set --serials, --avds, IRIS_ANDROID_E2E_SERIALS, or IRIS_ANDROID_E2E_AVDS."
        )
    return selected[:3]
...
def build_config(args: argparse.Namespace, artifact_dir: Path) -> Path:
    ...
    relay, reverse_relay = relay_config(args, artifact_dir, has_android_serial=has_android_serial)
    config: dict[str, Any] = {
        "name": f"{args.platform}-group-membership-{artifact_dir.name}",
    ...
    config_path = artifact_dir / f"{args.platform}-group-membership-config.json"
...
def main() -> int:
    ...
    artifact_dir = (args.artifact_dir or Path(f"/tmp/iris-{args.platform}-group-membership-{suffix}")).resolve()
    ...
    try:
        ...
        result["relay_mode"] = "local" if scenario.relay_config().get("start", True) else "public"
        result["relay_urls"] = scenario.relay_url()
        result["android_relay_urls"] = scenario.android_relay_url()
        write_json(artifact_dir / f"{args.platform}-group-membership-summary.json", result)
        ...
        write_json(artifact_dir / f"{args.platform}-group-membership-summary.json", failure)
        raise
    finally:
