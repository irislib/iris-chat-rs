...
from mixed_group_admin_matrix import run_flow, write_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Same-platform group admin E2E.")
...
def relay_config(args: argparse.Namespace, artifact_dir: Path, *, has_android_serial: bool) -> tuple[dict[str, Any], bool]:
        ...
        return (
            {
                ...
                "set_id": f"{args.platform}-group-admin-public-{artifact_dir.name}",
            },
            False,
        )
    ...
    relay = {
        "start": True,
        "port": port,
        "label": f"iris.{args.platform}-group-admin.{artifact_dir.name}.relay",
        "drop_file": str(artifact_dir / "scenario" / "drop-events.txt"),
        "log_file": str(artifact_dir / "scenario" / "relay.log"),
        "set_id": f"{args.platform}-group-admin-{artifact_dir.name}",
        "bind_host": "0.0.0.0",
...
def build_config(args: argparse.Namespace, artifact_dir: Path) -> Path:
    ...
    config: dict[str, Any] = {
        "name": f"{args.platform}-group-admin-{artifact_dir.name}",
    ...
    config_path = artifact_dir / f"{args.platform}-group-admin-config.json"
...
def main() -> int:
    ...
    artifact_dir = (args.artifact_dir or Path(f"/tmp/iris-{args.platform}-group-admin-{suffix}")).resolve()
    ...
    try:
        ...
        result["relay_mode"] = scenario.relay_config().get("start", True) and "local" or "public"
        result["relay_urls"] = scenario.relay_url()
        result["android_relay_urls"] = scenario.android_relay_url()
        write_json(artifact_dir / f"{args.platform}-group-admin-summary.json", result)
        ...
        write_json(artifact_dir / f"{args.platform}-group-admin-summary.json", failure)
        raise
    finally:
