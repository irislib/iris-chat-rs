...
EXPECTED = {
    "nvpn-fips-core": "0.4.74",
    "nvpn-fips-endpoint": "0.4.74",
    "nvpn-fips-tcp": "0.2.1",
    "nvpn-fips-tcp-endpoint": "0.2.10",
    "hashtree-config": "0.2.83",
    "hashtree-core": "0.2.89",
    "hashtree-fips-transport": "0.4.13",
    "hashtree-network": "0.2.87",
    "nostr-pubsub": "0.1.13",
    "nostr-pubsub-fips": "0.4.17",
...
class DependencyLockConsistencyTests(unittest.TestCase):
    ...
    def test_core_manifest_pins_gated_fips_stack_exactly(self):
        ...
        self.assertRegex(manifest, r'(?m)^fips-core = \{ package = "nvpn-fips-core", version = "=0\.4\.74",', "fips-core must stay on the gated release")
        self.assertRegex(manifest, r'(?m)^fips-endpoint = \{ package = "nvpn-fips-endpoint", version = "=0\.4\.74" \}$', "fips-endpoint must stay on the gated release")
        self.assertRegex(manifest, r'(?m)^fips-tcp = \{ package = "nvpn-fips-tcp", version = "=0\.2\.1" \}$', "fips-tcp must stay on the gated release")
        self.assertRegex(manifest, r'(?m)^fips-tcp-endpoint = \{ package = "nvpn-fips-tcp-endpoint", version = "=0\.2\.10" \}$', "fips-tcp-endpoint must stay on the gated release")
        ...
        self.assertRegex(manifest, r'(?m)^hashtree-fips-transport = "=0\.4\.13"$', "Hashtree/FIPS transport must stay on the gated release")
        ...
        self.assertRegex(manifest, r'(?m)^nostr-pubsub-fips = "=0\.4\.17"$', "nostr-pubsub-fips must stay on the gated release")
