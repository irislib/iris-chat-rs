...
EXPECTED = {
    "nvpn-fips-core": "0.4.76",
    "nvpn-fips-endpoint": "0.4.76",
    "nvpn-fips-tcp": "0.2.1",
...
class DependencyLockConsistencyTests(unittest.TestCase):
    ...
    def test_core_manifest_pins_gated_fips_stack_exactly(self):
        ...
        self.assertRegex(manifest, r'(?m)^fips-core = \{ package = "nvpn-fips-core", version = "=0\.4\.76",', "fips-core must stay on the gated release")
        self.assertRegex(manifest, r'(?m)^fips-endpoint = \{ package = "nvpn-fips-endpoint", version = "=0\.4\.76" \}$', "fips-endpoint must stay on the gated release")
        ...
        self.assertRegex(manifest, r'(?m)^nostr-pubsub-fips = "=0\.4\.19"$', "nostr-pubsub-fips must stay on the gated release")
