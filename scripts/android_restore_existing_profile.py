...
def discover_avds(limit: int) -> list[str]:
    ...
    if len(avds) < limit:
        raise SystemExit(
            f"Need {limit} Android AVDs or serials for the restore flow; found {len(avds)}. "
...
def select_android_targets(serials: list[str], avds: list[str]) -> list[dict[str, str]]:
    ...
    if len(selected) < 2:
        raise SystemExit(
            "Need two Android targets for the restore flow. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Restore an existing profile-bearing user with a secret key, then message.")
    ...
    parser.add_argument("--android-relay-url")
...
def build_config(args: argparse.Namespace, artifact_dir: Path) -> Path:
    ...
    if args.relay_mode == "public":
        ...
        relay_url = args.android_relay_url or args.relay_url or args.public_relays
        relay_config = {
            "start": False,
            "port": port,
            "android_url": relay_url,
            "url": relay_url,
            "set_id": f"android-restore-profile-public-{artifact_dir.name}",
        }
        reverse_relay = False
    else:
        port = args.relay_port or free_tcp_port()
        relay_url = args.android_relay_url or args.relay_url or (
            f"ws://127.0.0.1:{port}" if has_serial else f"ws://10.0.2.2:{port}"
        )
        relay_config = {
            "port": port,
            "label": f"iris.android-restore-profile.{artifact_dir.name}.relay",
            "drop_file": str(artifact_dir / "scenario" / "drop-events.txt"),
            "log_file": str(artifact_dir / "scenario" / "relay.log"),
            "set_id": f"android-restore-profile-{artifact_dir.name}",
            "bind_host": "0.0.0.0",
            "android_url": relay_url,
            "url": relay_url,
        }
    ...
    config = {
        "name": f"android-restore-profile-{artifact_dir.name}",
    ...
    config_path = artifact_dir / "android-restore-profile-config.json"
...
def run_flow(scenario: Scenario, artifact_dir: Path) -> dict[str, Any]:
    ...
    alice_to_bob = f"restore-existing-a2b-{marker}"
    bob_to_alice = f"restore-existing-b2a-{marker}"
    ...
    restart_android_app(scenario, "alice1")
    ...
    result = {
        ...
        "relay_urls": scenario.android_relay_url(),
        "setup": "android",
        "state": str(scenario.state_path),
    }
    write_json(artifact_dir / "android-restore-profile-summary.json", result)
    return result
