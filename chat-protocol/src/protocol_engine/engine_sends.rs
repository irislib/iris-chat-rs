impl ProtocolEngine {
    ...
    pub fn ingest_nostr_identity_roster_op_event(
        &mut self,
        event: &Event,
    ) -> anyhow::Result<Option<ProtocolDeviceRosterIngestionResult>> {
        if !is_nostr_identity_roster_op_event(event) {
            return Ok(None);
        }
        let Some(profile_id) = nostr_identity_roster_profile_id(event) else {
            return Ok(None);
        };
        ...
        let retry_batch =
            match self.ingest_app_keys_snapshot(owner_pubkey, app_keys.clone(), created_at) {
                Ok(retry_batch) => retry_batch,
                Err(error) => {
                    self.restore_checkpoint(checkpoint);
                    return Err(error);
        ...
        Ok(Some(ProtocolDeviceRosterIngestionResult {
            owner_pubkey,
            app_keys,
            created_at,
            retry_batch,
    ...
    fn remember_nostr_identity_roster_op_event(
        &mut self,
        profile_id: &str,
        event: &Event,
    ) -> (PublicKey, Vec<Event>, u64) {
        let history = self
            .nostr_identity_roster_histories
            .entry(profile_id.to_string())
            .or_default();
        let owner_pubkey = *history.owner_pubkey.get_or_insert(event.pubkey);
        if !history.events.iter().any(|existing| existing.id == event.id) {
            history.events.push(event.clone());
            history.events.sort_by(|left, right| {
                left.created_at
                    .as_secs()
            ...
            let excess = history
                .events
                .len()
                .saturating_sub(NOSTR_IDENTITY_ROSTER_EVENT_HISTORY_LIMIT);
            if excess > 0 {
                history.events.drain(0..excess);
            }
        }
        ...
        (owner_pubkey, history.events.clone(), created_at)
    }
